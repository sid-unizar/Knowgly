...
def coordinate_ascent(adjust_parameters):
    ...
    weights = [1.0, random.uniform(0.25, 0.75), random.uniform(0.0, 0.25), random.uniform(0.0, 0.25),
               random.uniform(0.0, 0.25)]

    k1 = 0.9  # Default elasticsearch value (galago's is 0.5, which is quite bad)
    b = 0.4  # Default galago value
    ...
    for iteration in range(num_iterations):
        ...
        if weights[0] - STEP_SIZE >= 0.0 and (INDEX != "lucene" or weights[0] - STEP_SIZE >= 1.0):
            scores.append(
                common.search_with_field_weights_5(weights[0] - STEP_SIZE, weights[1], weights[2], weights[3],
                                                   weights[4], k1,
                                                   b, QUERIES_FILE, QRELS_FILE, INDEX))
        else:
            scores.append(0.0)
        ...
        if weights[1] - STEP_SIZE >= 0.0 and (INDEX != "lucene" or weights[1] - STEP_SIZE >= 1.0):
            scores.append(
                common.search_with_field_weights_5(weights[0], weights[1] - STEP_SIZE, weights[2], weights[3],
                                                   weights[4], k1,
                                                   b, QUERIES_FILE, QRELS_FILE, INDEX))
        else:
            scores.append(0.0)
        ...
        if weights[2] - STEP_SIZE >= 0.0 and (INDEX != "lucene" or weights[2] - STEP_SIZE >= 1.0):
            scores.append(
                common.search_with_field_weights_5(weights[0], weights[1], weights[2] - STEP_SIZE, weights[3],
                                                   weights[4], k1,
                                                   b, QUERIES_FILE, QRELS_FILE, INDEX))
        else:
            scores.append(0.0)
        ...
        if weights[3] - STEP_SIZE >= 0.0 and (INDEX != "lucene" or weights[3] - STEP_SIZE >= 1.0):
            scores.append(
                common.search_with_field_weights_5(weights[0], weights[1], weights[2], weights[3] - STEP_SIZE,
                                                   weights[4], k1,
                                                   b, QUERIES_FILE, QRELS_FILE, INDEX))
        else:
            scores.append(0.0)
        ...
        if weights[4] - STEP_SIZE >= 0.0 and (INDEX != "lucene" or weights[4] - STEP_SIZE >= 1.0):
            scores.append(
                common.search_with_field_weights_5(weights[0], weights[1], weights[2], weights[3],
                                                   weights[4] - STEP_SIZE, k1,
                                                   b, QUERIES_FILE, QRELS_FILE, INDEX))
        else:
            scores.append(0.0)
...
def run_coordinate_ascent(adjust_parameters, filename_prefix):
    ...
    common.save_results_5(best_weights[0], best_weights[1], best_weights[2], best_weights[3], best_weights[4],
                          best_k1_attempts,
                          best_b_attempts, filename_prefix, QUERIES_FILE)
...
if __name__ == "__main__":
    ...
    for train_index, test_index in kf.split(queries):
        ...
        fold_scores.append(
            common.search_with_field_weights_5(weights[0], weights[1], weights[2], weights[3], weights[4], k1, b,
    ...
    common.save_results_5(best_weights[0], best_weights[1], best_weights[2], best_weights[3], best_weights[4],
                          best_k1_attempts,
                          best_b_attempts, "ca_results", QUERIES_FILE, INDEX)
