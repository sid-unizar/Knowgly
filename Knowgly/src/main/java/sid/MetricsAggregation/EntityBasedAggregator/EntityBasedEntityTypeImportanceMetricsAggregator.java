...
/**
 * An entity based metrics aggregator, using the entityTypeImportance metric
 */
public class EntityBasedEntityTypeImportanceMetricsAggregator extends EntityBasedMetricsAggregator {
    /**
     * Basic constructor, with aggregator combination. Initializes the clustering configuration via its configuration file
     */
    public EntityBasedEntityTypeImportanceMetricsAggregator(String entityURI,
    ...
    public void cacheAllMetrics(LocalHDTSPARQLEndpoint endpoint, VirtualDocumentTemplate globalTemplate) throws IOException {
        super.cacheAllMetrics(endpoint, "entityTypeImportance", globalTemplate);
    }
