...
/**
 * An entity based metrics aggregator, using the entropyTypeImportance metric
 */
public class EntityBasedEntropyTypeImportanceMetricsAggregator extends EntityBasedMetricsAggregator {
    /**
     * Basic constructor, with aggregator combination. Initializes the clustering configuration via its configuration file
     */
    public EntityBasedEntropyTypeImportanceMetricsAggregator(String entityURI,
    ...
    public void cacheAllMetrics(LocalHDTSPARQLEndpoint endpoint, VirtualDocumentTemplate globalTemplate) throws IOException {
        super.cacheAllMetrics(endpoint, "entropyTypeImportance", globalTemplate);
    }
